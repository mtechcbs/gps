...
switch($type){
	...
	$upload_exts = end(explode(".", $_FILES["picture"]["name"]));
		...
		$data["createGPSID"]["address"]=$fval["address"];
		$data["createGPSID"]["city"]=$fval["city"];
		$data["createGPSID"]["state"]=$fval["state"];
		$data["createGPSID"]["country"]=$fval["country"];
		$data["createGPSID"]["pincode"]=$fval["pincode"];
...
case "state":
 ...
 }else{	   include "layouts/default/create_firm.html";
	   }		
